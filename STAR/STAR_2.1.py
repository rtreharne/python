...
class Window():
    ...
    def export(self, widget):
        ...
        savetxt('data.txt', transpose([data]))
...
#Materials Window and functions -----------------------------------------------
    ...
    def model_win(self, widget):
        ...
        self.param_box = gtk.ScrolledWindow()
        self.param_box.set_policy(gtk.POLICY_AUTOMATIC, gtk.POLICY_AUTOMATIC)
        ...
        self.treeview.append_column( col)
        cell = gtk.CellRendererText()
        #cell.set_property("adjustment", gtk.Adjustment(lower=0, upper = 10, step_incr=0.1))
        ...
        box4 = gtk.HBox()
        box4.set_size_request(500,400)
        box4.pack_start(self.mod_label)

        box5 = gtk.VBox()
        box5.pack_start(box1)
        box5.pack_start(box2)
        box5.pack_start(box3)
        box5.pack_start(box4)

        self.model_win.add(box5)
        self.model_win.show_all()

    def add_model(self, widget):
        self.mainbox_index = self.mainbox.get_active()
        print self.mainbox_index
        #self.model.append(["const n", 2.0, True])
   
         
    def _download_toggled( self, w, row, column):
        self.model[row][column] = not self.model[row][column]
        print self.model[row][column]
