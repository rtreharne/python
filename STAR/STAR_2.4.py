...
class Window():
    ...
    def av_T(self):
        ...
        print "T = ", self.average_T

    def av_R(self):
        area = 0
        
        for i in range(self.indmin+1, self.indmax):
            area+=real((self.R[i-1]+self.R[i])*(self.lam[i]-self.lam[i-1])/2)
        self.average_R = area/(self.xmax-self.xmin)
        print "R = ", self.average_R
