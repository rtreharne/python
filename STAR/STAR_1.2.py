...
class Window():
    ...
    def plot_nk(self,widget):
        ...
        self.ln = self.ax1.plot(self.lam, real(self.nk[self.index]), '-', color = 'red', linewidth = 2)
        ...
        self.lk = self.ax2.plot(self.lam, -imag(self.nk[self.index]), '-', color = 'green', linewidth = 2)
        self.ax2.set_ylabel('k', fontsize = 16, color = 'red')
        #self.ax2.set_ylim(0,-max(imag(self.nk[self.index])))
        self.ax2.set_xlim(min(self.lam),max(self.lam))
        self.canvas.draw()
    ...
    def plot_e(self, widget):
        ...
        self.fig1.clf()
        self.ax1 = self.fig1.add_subplot(111)
        self.ln = self.ax1.plot(self.lam, e1, '-', color = 'red', linewidth = 2)
        self.ax1.set_xlabel('$\lambda$ (nm)', fontsize = 16)
        self.ax1.set_ylabel('e1', fontsize = 16, color = 'red')
        self.ax1.set_xlim(min(self.lam),max(self.lam))
        #self.ax1.set_ylim(min(real(self.nk[self.index])), max(real(self.nk[self.index])))
        self.ax2 = self.ax1.twinx()
        self.lk = self.ax2.plot(self.lam, e2, '-', color = 'green', linewidth = 2)
    ...
    def plot_TR(self, widget):
        ...
        self.ln2 = self.ax1.plot(self.lam, real(self.T2), '--', color = 'purple', linewidth = 1)
        self.ax1.set_xlabel('$\lambda$ (nm)', fontsize = 16)
    ...
    def __init__(self):
        ...
        self.plot_TR_but.connect("clicked", self.plot_TR)
        self.plot_TR_but.set_sensitive(False)
